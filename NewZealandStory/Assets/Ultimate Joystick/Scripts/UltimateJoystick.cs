...
public class UltimateJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
	...
	// FUNCTIONALITY //
	public bool throwable = false, draggable = false;
	...
	public void OnPointerUp ( PointerEventData touchInfo )
	{
		...
		if( dynamicPositioning == true || disableVisuals == true || draggable == true )
		{
	...
	void UpdateJoystick ( PointerEventData touchInfo )
	{
		...
		// Clamp the vector by the radius.
	...
	/// <summary>
	/// Returns a Vector2 value containing the horizontal and vertical values of the Ultimate Joystick.
	/// </summary>
	Vector2 GetPositionValues ()
	{
	...
	{
		float temp = GetPositionValues().x;
	...
	{
		float temp = GetPositionValues().y;
		if( Mathf.Abs( temp ) <= deadZone )
			temp = 0.0f;
