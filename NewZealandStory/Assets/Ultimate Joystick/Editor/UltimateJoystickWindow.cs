...
public class UltimateJoystickWindow : EditorWindow
{
	...
	void OnEnable ()
	{
		style = ( GUISkin )Resources.Load( "UltimateJoystickEditorSkin" );

		scriptReference = ( Texture2D )Resources.Load( "UJ_ScriptRef" );
	...
	void DocumentationPage ()
	{
		...
		EditorGUILayout.LabelField( Indent + "The following functions can be referenced from your scripts without the need for an assigned local Ultimate Joystick variable. However, each function must have the targeted Ultimate Joystick name in order to find the correct Ultimate Joystick in the scene. Each example code provided uses the name 'Movement' as the joystick name.", paragraphStyle );// EDIT: 
