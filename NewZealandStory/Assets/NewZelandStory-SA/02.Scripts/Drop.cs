...
public class Drop : MonoBehaviour {
	...
	public Enemy enemy;							//Enemy 스크립트 받아옴
    ...
    public void Update() {
        ...
        if (enemy.dead == true)
		{
			if (drop == false) {
				...
				//drop할 좌표받아오기
				dropPosX = enemy.lastpositionX;
                dropPosY = enemy.lastpositionY;
	...
	public IEnumerator DropTheItem()
	{
		...
		Vector3 dropPos = new Vector3 (dropPosX,  dropPosY+0.5f, 0f);

		randomIndex = Random.Range (0, DropItems.Length);							//랜덤으로 뽑기

		if(activebomb.enabled == true&&randomIndex==10)
			yield return null;
				
		else
        //	Instantiate (DropItems [randomIndex], dropPos, Quaternion.identity);    //아이템생성
        {
            GameObject item = Instantiate(DropItems[randomIndex], dropPos, Quaternion.identity);
            item.transform.parent = transform.root.GetComponent<Transform>();
        }
